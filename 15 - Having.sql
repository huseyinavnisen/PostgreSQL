...
from workers
group by name
having sum(salary)>2500;   ---> grup by ardından where yerine having kullanılır.
...
--distinct ifadesi çağrılan terimlerden tekrralı olanların sadece birincisini alır.
...
--1.Yol
select concat(name,'   ',state) as isim_sehir
from workers
